...
let double_selectively array indices : unit = List.iter indices ~f:(fun x -> array.(x) <- array.(x)*2)
...
let double_matrix matrix : unit = Array.iteri matrix ~f:(fun i arr -> Array.iteri arr ~f:(fun j x -> matrix.(i).(j) <- x*2))
